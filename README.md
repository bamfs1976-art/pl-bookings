...
Two details are load-bearing on a phone. The bar is **sticky**, because `index.html` restores its scroll position on load — it opens 131px down, so a bar in normal flow was already off-screen when the page appeared, on the one page that most needed it. And below 560px the row **scroll-snaps** rather than abbreviating: at full length it needs 625px against the widest iPhone's 430, so "La Liga" and "Today" sat off the right edge on every handset. Shortening the labels was tried first and abandoned — the two items that lost their names were the two nobody could then identify — so every item keeps its label, the row snaps, and the desk you are on is scrolled into view on load. (This paragraph described the abandoned version until 2026-09-05; the rule reversing it is the second `@media (max-width:560px)` block in `assets/tw.css`, and it carries the reasoning.)
...
Permitted data sources are: the MIT-licensed [epldata](https://github.com/pssguy/epldata) package, the [DataHub English Premier League mirror](https://github.com/datasets/football-datasets) of football-data.co.uk (PDDL), [openfootball](https://github.com/openfootball) (CC0), and the official FPL API through this site's own proxy. **Nothing here is fetched, scraped or derived from FBref, WhoScored, FootyStats, Understat or any bookmaker.** Where a feature needs data only those carry, it takes the number by hand and says so — the promoted clubs' fouls-drawn figure goes in through the Import view, and the per-player backtest simply does not run. The full position is in the app's **Sources & licences** view.
