...
| `check-ucl.mjs` (added 9 September 2026) | A Champions League tie ships with one club unheld, so a card prices a real XI against last season's minutes; a club is filed under the wrong desk; a whole league phase carries the feed's provisional kick-off instead of no date at all. | Join | No: reads the tie list against all three desks' own data files. |
| `check-encoding.mjs` (added 9 September 2026) | A player's name ships double-encoded, so a desk and a share card spell it "Dani MartÃ­nez". API-Football serves a few names that way; the harvest repairs them on arrival and this catches a regression in that repair, or a new source that bypasses it. | Number | No: scans every shipped data file, page and module. |
| `check-photos.mjs` (added 10 September 2026) | A desk ships a dataset that has lost its player photographs, which renders and prices perfectly and so reports itself nowhere. The Championship fell from 575 faces to 126 in one refresh and it surfaced six hours later in another workflow, with the bad data already on main. | Number | No: counts faces in the four shipped datasets. |
