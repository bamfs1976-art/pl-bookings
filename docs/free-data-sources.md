...
> **false now**: `eflc.html` and `laliga.html` both load `assets/livecards.js`
> and poll `/api/live-cards` through `LiveCards.pollLoop`, so both carry a live
> in-play card ticker. What is still frozen on those two desks is
> **availability** — `inj` is baked into the shipped frames and refreshed by the
> daily cron, not by the page — so §4's recommendation (stamp the freshness)
> stands and is the part still worth doing. The word "photographs", wherever it
> appears below, should now be read as "live for cards, day-old for
> availability". Everything else in §1's table was re-checked on merging and
> still holds.
>
...
| Gap | State |
|---|---|
| ~~Championship and La Liga desks are static~~ | **Closed since.** Both now poll `/api/live-cards`. Availability on them is still day-old — see the note above |
