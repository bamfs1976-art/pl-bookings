...
/* THE FLAGS THAT TAKE A VALUE, in one place. The history path is the first
   argument that is neither a flag nor the value of one of these. The list used
   to name --label and --out and not --report, so `--report backtest_report.md`
   was read as the history path: the script looked for match history inside
   the report it was meant to write, printed "No match history at
   backtest_report.md", and the workflow's `|| true` swallowed the exit. Every
   scheduled run from July to September harvested the history correctly and
   wrote no report. A flag added here without being added to this list
   reproduces that bug, which is why tests/test-libs.mjs runs the script with
   the workflow's own argument shape. */
const VALUED = ['--label', '--out', '--report'];
const flag = (name) => { const i = argv.indexOf(name); return i > -1 ? argv[i + 1] : null; };
const path = argv.find((a, i) => !a.startsWith('--') && !VALUED.includes(argv[i - 1]))
  || join(root, 'data', 'match_history.json');
const label = flag('--label') || 'Premier League';
const outFile = flag('--out');
const reportFile = flag('--report');
const reportPath = () => (reportFile.startsWith('/') ? reportFile : join(root, reportFile));
...
const WARMUP = rounds.length ? Math.max(rounds[0] + 3, rounds[Math.min(4, rounds.length - 1)]) : null;
...
if (!preds.base.length) {
  /* NOT ENOUGH ROUNDS YET. Early in a season this is the expected state, not
     a failure: the walk-forward scores round R on a fit over rounds before R,
     and needs at least 200 training rows behind a warm-up. With --report the
     report IS the output, so it is written, dated and stamped with what the
     history held, and the exit is clean: a committed file saying "three
     rounds, first scorable round is five" is checkable, while a step that
     failed quietly every morning is not. At a terminal, with no report asked
     for, the old behaviour stands: say so and exit non-zero. */
  const booked = rows.reduce((s2, r) => s2 + (r.y ? 1 : 0), 0);
  const msg = `Not enough rounds to backtest: ${rows.length} match rows over ${rounds.length} `
    + `round(s)${rounds.length ? ` (${rounds[0]}-${rounds[rounds.length - 1]})` : ''}; the first `
    + `scorable round is ${WARMUP == null ? 'unknown' : WARMUP} and it needs 200 training rows behind it.`;
  if (reportFile) {
    const md = [
      '# Backtest report',
      '',
      ...
      '**No scoring run yet.**',
      '',
      `**${label}** · source \`${path.replace(root + '/', '')}\` · generated ${new Date().toISOString()}`,
      '',
      `The history holds **${rows.length} match rows over ${rounds.length} round(s)**`
        + `${rounds.length ? ` (rounds ${rounds[0]}-${rounds[rounds.length - 1]})` : ''}, `
        + `${booked} of them booked (${rows.length ? (100 * booked / rows.length).toFixed(1) : '0.0'}%).`,
      '',
      `The walk-forward scores a round on a model fitted to the rounds before it, after a`,
      `warm-up. The first round it could score is **round ${WARMUP == null ? '?' : WARMUP}**, and it`,
    ...
    console.log(msg);
    console.log(`report written to ${reportFile} (no scoring run yet)`);
    process.exit(0);
  }
  console.error(msg);
  process.exit(1);
